...
function normalizeCycle(
  ...
  /*
   * Recalcula somente o uso das categorias
...
function getSalaryBalance() {
  ...
  /*
   * Todo gasto originado no Salário
   * reduz o saldo do Salário.
   */
  ...
  /*
   * Toda transferência Salário → Reserva
   * também reduz o saldo disponível.
   *
   * A transferência não é gasto.
   */
...
/*
 * Transferência real para a Reserva.
 *
 * SALÁRIO:
 * - não altera salary.reference;
 * - registra a transferência;
 * - getSalaryBalance() passa a descontá-la.
 *
 * EXTRA:
 * - desconta imediatamente do saldo persistente;
 * - registra a transferência.
 *
 * RESERVA:
 * - recebe o valor.
...
function saveToReserve(
  ...
  if (
    ...
    throw new Error(
      "Não existe um ciclo financeiro ativo."
    );
   ...
   * EXTRA é saldo persistente.
   *
   * Portanto precisa ser descontado
   * diretamente de state.extra.balance.
   */
  if (
    origin === "extra"
  ) {
  ...
  /*
   * SALÁRIO não é descontado diretamente
   * de state.salary.reference.
   *
   * O saldo disponível é calculado
   * usando as transferências registradas.
   */
  ...
  /*
   * Finalmente adiciona o dinheiro
   * à Reserva.
   */
  state.reserve.balance =
    roundMoney(
...
/*
 * Retirada da Reserva é GASTO.
 *
 * O dinheiro NÃO retorna para Extra.
 *
 * Fluxo:
 *
 * Reserva
 ...
 * categoria Outros
 *
 * Não existe alteração em state.extra.balance.
 */
function withdrawFromReserve(
  ...
  /*
   * A retirada é um gasto real.
   *
   * Não retorna para Extra.
   */
  createExpenseRecord(
    "reserve",
    amount,
...
function launchExpense(
   ...
   * Extra é saldo persistente.
   *
   * O Salário é calculado por eventos.
   */
  if (
    origin ===
    "extra"
...
function switchTab(
  ...
  if (
    ...
    if (btnExtrato) {
      ...
      btnExtrato.classList.add(
        "active"
      );

      btnExtrato.style.color =
        "#ffffff";
  ...
  if (previousLabel) {
    ...
    if (
      ...
      previousLabel.textContent =
        `Mês anterior (${formatDate(prev.startDate)})`;
...
function showReserveSaveForm() {
  ...
  /*
   * Mostra os saldos reais disponíveis
   * no momento em que o formulário é aberto.
   */
  setText(
    "reserve-salary-available",
    formatMoney(
      getSalaryBalance()
    )
  );
...
function openPreviousCycle() {
  ...
  if (
    ...
    alert(
      "Ainda não existe um ciclo anterior registrado."
    );
...
function openSetupCategoryEditor(
  ...
  if (
    ...
    alert(
      "A categoria Outros não possui limite de gastos."
    );
